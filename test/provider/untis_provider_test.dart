  ...
  group('Untis Provider:', () {
    ...
    setUp(() {
      ...
      when(
        ...
        (i) async =>
            (i.namedArguments[const Symbol('startDate')] ==
    ...
    test('Initial values are correct', () {
      ...
      expect(
        untisProvider.untisSubjectStatus,
    ...
    test(
      'should select the earliest next lesson which is not cancelled',
      () async {
        int listenerCallsCounter = 0;
    ...
    Future<void> currentSubjectTest(
      ...
      // setup
      final List<UntisPeriod> todayPeriods = endDates
          .map((date) => createPeriodFromDate(date))
          .toList();
      when(todayTimetable.periods).thenReturn(todayPeriods);
      // test
    ...
    test(
      'should return the most recent subject when there are more than one',
      () async {
        await currentSubjectTest([
    ...
    test(
      'should return no current subject when there is no valid period ',
      () async {
        // setup
        final todayPeriods = <UntisPeriod>[
          createMockUntisPeriod(

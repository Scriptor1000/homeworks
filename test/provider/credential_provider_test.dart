...
void main() {
  group('Credential Provider: ', () {
    ...
    setUp(() {
      ...
      when(firestoreCredentials.loadCredentials(userPassword))
          .thenAnswer((_) async => testCredentials);
      when(firestoreCredentials.loadCredentials(falsePassword))
          .thenThrow(Exception('Decryption failed'));

      when(storage.read(key: CredentialProvider.credentialsKey))
          .thenAnswer((_) async => testCredentials.toJsonString());
      when(itemFactory.untisCredentialsFromJSON(testCredentials.toJsonString()))
          .thenReturn(testCredentials);
      when(itemFactory.createUntisSession(testCredentials))
    ...
    test('should initializing by loading local and checking online', () async {
      ...
      verify(itemFactory
              .untisCredentialsFromJSON(testCredentials.toJsonString()))
          .called(1);
    ...
    test('should save credentials local and create session on online load',
        () async {
      // test
    ...
    test('should handle error on wrong password for online load', () async {
      ...
      await credentialProvider
          .loadCredentialsOnline(falsePassword)
          .catchError((e) {
      ...
      verifyNever(storage.write(
          key: CredentialProvider.credentialsKey, value: anyNamed('value')));
    });

    test('should raise error if no credentials are found on online load ',
        () async {
      // test
      await credentialProvider
      ...
      expect(credentialProvider.hasCredentials, isFalse);
      expect(credentialProvider.credentials, isNull);
      verify(firestoreCredentials.loadCredentials(nonExistingPassword))
          .called(1);
      verifyNever(itemFactory.createUntisSession(any));
      verifyNever(storage.write(
          key: CredentialProvider.credentialsKey, value: anyNamed('value')));
    ...
    test('should save credentials and change online status on upload',
        () async {
      // setup
      await credentialProvider.setCredentials(testCredentials);
      // test
      ...
      verify(firestoreCredentials.saveCredentials(
              testCredentials, userPassword))
          .called(1);
    });

    test('should not raise error if asked to upload non existend credentials',
        () async {
      // test
      await credentialProvider.uploadCredentialsOnline(userPassword);
      // verify
